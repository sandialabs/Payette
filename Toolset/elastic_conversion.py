...
NAM_MAP = {"lam": 0, "G": 1, "E": 2, "nu": 3, "K": 4, "H": 5,
           "ko": 6, "cl": 7, "ct": 8, "co": 9, "cr": 10, "rho": 11,}

def compute_elastic_constants(*args):
    """
    PURPOSE
    ...
    return {"lam":lam,"G":G,"E":E,"nu":nu,"K":K,"H":H,"ko":ko,
            "cl":cl,"ct":ct,"co":co,"cr":cr,"rho":rho, "retcode": 0}
...
def elastic_param_conversion(argv):
    ...
    parser.add_option("--bulk",dest="bulk",action="store",default=None,
                      help=("bulk modulus = E/3/(1-2nu) = lam + 2G/3 "
                            "[default: %default]"))
    parser.add_option("--constrained",dest="constrained",action="store",default=None,
    ...
    if opts.lam is not None:
        ui[NAM_MAP["lam"]] = float(opts.lam)
    if opts.shear is not None:
        ui[NAM_MAP["G"]] = float(opts.shear)
    if opts.youngs is not None:
        ui[NAM_MAP["E"]] = float(opts.youngs)
    if opts.poissons is not None:
        ui[NAM_MAP["nu"]] = float(opts.poissons)
    if opts.bulk is not None:
        ui[NAM_MAP["K"]] = float(opts.bulk)
    if opts.constrained is not None:
        ui[NAM_MAP["H"]] = float(opts.constrained)
    if opts.sigratio is not None:
        ui[NAM_MAP["ko"]] = float(opts.sigratio)
    if opts.longwvspd is not None:
        ui[NAM_MAP["cl"]] = float(opts.longwvspd)
    if opts.tranwvspd is not None:
        ui[NAM_MAP["ct"]] = float(opts.tranwvspd)
    if opts.bulkwvspd is not None:
        ui[NAM_MAP["co"]] = float(opts.bulkwvspd)
    if opts.thinwvspd is not None:
        ui[NAM_MAP["cr"]] = float(opts.thinwvspd)
...
def ask_input(query, defnam, defval):
    ...
    while True:
        ...
        if nam is None:
            print("\nunknown constant %s\nvalid entries are %s\ntry again"
                  %("None",', '.join(NAM_MAP.keys())))
            continue

        elif nam not in NAM_MAP:
            print("\nunknown constant %s\nvalid entries are %s\ntry again"
                  %(nam,', '.join(NAM_MAP.keys())))
            continue
...
def print_elastic_constants(econsts):
    emods = [x for x in econsts if x in
             sorted(NAM_MAP, key=NAM_MAP.__getitem__)[:7]]
