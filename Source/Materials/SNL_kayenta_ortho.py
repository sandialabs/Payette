...
try:
    import Source.Materials.Library.kayenta_ortho as mtllib
    imported = True
except:
    imported = False
    pass
...
attributes = {"payette material":True,
              "name":"kayenta_ortho",
              "fortran source":True,
              "build script":os.path.join(Payette_Materials_Fortran,"Kayenta_Orthotropic/build.py"),
              "aliases":["orthotropic kayenta"],
              "material type":["mechanical"]
              }

class KayentaOrthotropic(ConstitutiveModelPrototype):
    '''
    CLASS NAME
       KayentaOrtho

    PURPOSE
       Constitutive model for a KayentaOrtho material. When instantiated, the
       KayentaOrtho material initializes itself by first checking the user input
    ...
    FORTRAN
       The core code for the KayentaOrtho material is contained in
    ...
    def __init__(self):
        ...
        nparams = nbasicinput + njntinputs + neos
        ...
        self.registerParameter("CKN02",71,aliases=['CKN02', 'CN2'])
        self.registerParameter("CKN03",72,aliases=['CKN03', 'CN3'])
        self.registerParameter("VMAX1",73,aliases=['VMAX1', 'VM1'])
        self.registerParameter("VMAX2",74,aliases=['VMAX2', 'VM2'])
        self.registerParameter("VMAX3",75,aliases=['VMAX3', 'VM3'])
        self.registerParameter("SPACE1",76,aliases=['SPACE1'])
        self.registerParameter("SPACE2",77,aliases=['SPACE2'])
        self.registerParameter("SPACE3",78,aliases=['SPACE3'])
        self.registerParameter("SHRSTIFF1",79,aliases=['SHRSTIFF1', 'ST1'])
        self.registerParameter("SHRSTIFF2",80,aliases=['SHRSTIFF2', 'ST2'])
