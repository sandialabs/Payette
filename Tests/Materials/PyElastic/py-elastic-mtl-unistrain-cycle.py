...
Payette_test = True
name = 'py-elastic-mtl-unistrain-cycle'
fdir = os.path.dirname(os.path.realpath(__file__))
infile = '%s.inp'%(os.path.join(fdir,name))
outfile = '%s.out'%(name)
keywords = ['fast','verification','elastic','uniaxial strain',"python model",
            "pyelastic"]
...
date = 'June 24, 2011'
description = '''
    In this test, an elastic material (python elastic material) is
    cycled through a uniaxial strain deformation path in the
    following steps:
...
if __name__ == '__main__':
    ...
    if '--cleanup' in sys.argv:
        for ext in ['out','diff','log','prf','pyc','echo','prf','props']:
